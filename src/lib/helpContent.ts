...
const EN: HelpTopic[] = [
  ...
  },
  {
    ...
    section: "Git",
    title: "Git panel",
    body: "The Git tab in the sidebar (or the branch name in the status bar) shows your changes: stage/unstage per file or all at once, discard changes, commit (Ctrl+Enter in the message box), push and pull. Click a file to see its diff against HEAD; the History section (collapsible, own scroll, Load more) lists commits - click one to view its full patch. New files are listed one by one even when a whole folder is new, so nothing hides behind a folder row. Changed files get letters in the file tree and changed lines get colored marks in the editor gutter. Smart by default: committing with nothing staged stages everything first, and the first push of a new branch sets its upstream automatically. Works entirely without AI; with AI, the sparkle button writes the commit message from the staged (or unstaged) diff.",
...
const VI: HelpTopic[] = [
  ...
  },
  {
    ...
    section: "Git",
    title: "Panel Git",
    body: "Tab Git ở sidebar (hoặc bấm tên branch trên status bar) hiển thị thay đổi: stage/bỏ stage từng file hoặc tất cả, hủy thay đổi, commit (Ctrl+Enter trong ô nội dung), push và pull. Bấm vào file để xem diff so với HEAD; khu Lịch sử (gập/mở, cuộn riêng, nút Tải thêm) liệt kê commit - bấm vào để xem trọn patch. File mới được liệt kê từng file kể cả khi cả thư mục là mới, không có file nào nấp sau một dòng thư mục. File thay đổi có chữ trạng thái trong cây file, dòng thay đổi có vạch màu ở lề editor. Thông minh mặc định: commit khi chưa stage gì sẽ tự stage toàn bộ, push lần đầu của nhánh mới tự thiết lập upstream. Hoạt động hoàn toàn không cần AI; có AI thì nút lấp lánh tự viết nội dung commit từ diff (đã stage hoặc chưa).",
