...
export function McpModal({ onClose }: { onClose: () => void }) {
  ...
  /**
   * Header, one scrolling body, one form.
   *
   * The list of configured servers and the catalog are two views of the same
   * body rather than two stacked panels: as separate scroll regions they
   * fought over the dialog's height and pushed the form under the footer.
   */
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 pt-16" onClick={onClose}>
      <div
        className="flex max-h-[80vh] w-[640px] max-w-[92vw] flex-col rounded-xl border border-line bg-panel shadow-2xl"
        onClick={(e) => {
          e.stopPropagation();
        }}
      >
        ...
        {!catalogOpen && (
          <p className="shrink-0 border-b border-line px-4 py-2 text-[11px] text-muted">{t("mcp.hint")}</p>
        )}
        ...
        {catalogOpen && (
          <input
            ...
            className="shrink-0 border-b border-line bg-transparent px-4 py-2 text-[12px] outline-none placeholder:text-muted"
          />
        )}

        {error && (
          <div className="shrink-0 border-b border-danger/40 bg-danger/10 px-4 py-1.5 text-[12px] text-danger">
            {error}
          </div>
        )}

        <div className="min-h-24 flex-1 overflow-y-auto px-4 py-2">
          {catalogOpen ? (
            found === null ? (
              <>
        ...
        <div className="flex shrink-0 flex-col gap-2 border-t border-line px-4 py-3">
          ...
          />
          {/* The explanation sits on its own line: sharing a row with the
              buttons is what made it collide with them. */}
          <p className="text-[10px] text-muted">{t("mcp.footer")}</p>
          <div className="flex items-center gap-2">
            ...
            >
              <Search size={12} /> {catalogOpen ? t("mcp.backToList") : t("mcp.browseCatalog")}
            </button>
            <span className="flex-1" />
            <button
              onClick={() => void add()}
